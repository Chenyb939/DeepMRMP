...
def fit_model(trainX, trainY, valX, valY, cnn, i, type):
    checkpointer = ModelCheckpoint(filepath='./fig2/%s' % type + '/CNNGRU/model/' + str(type) + '_%d.h5' % i, verbose=1,
...
def fine_tuning(m1AtrainX, m1AtrainY, m1AvalX, m1AvalY, i, type, modelname):
    ...
    earlystopping = EarlyStopping(monitor='val_loss', mode='min', patience=2000)
...
def evaluate_model(modelname, testX, testY, i, type):
    ...
    fileX = open('./fig2/%s' % type + '/CNNGRU/pre_socre/pre_score%d.pickle' % i, 'wb')
    ...
    plt.savefig('./fig2/%s' % type + '/CNNGRU/curve/' + str(type) + 'Precision-Recall%d.png' % i)
    ...
    plt.savefig('./fig2/%s' % type + '/CNNGRU/curve/' + str(type) + 'ROC %d.png' % i)
...
def print_loss(fitHistory, i, type):
    ...
    plt.savefig('./fig2/%s' % type + '/CNNGRU/curve/' + str(type) + 'loss%d.png' % i)
...
def main(type):
    if os.path.exists('./fig2/%s' % type + '/CNNGRU/results.txt'):
        print("The result will be written in the 'results.txt' file")
    else:
        with open('./fig2/%s' % type + '/CNNGRU/results.txt', 'w') as file:
    ...
    for i in range(1, 5):
        ...
        history = fine_tuning(trainX, trainY, valX, valY, i, type, './m6A/CNNGRU/1_m6A.h5')
        ...
        pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model('./fig2/%s' % type + '/CNNGRU/model/' + str(type) + '_%d.h5' % i,
                                                                       testX, testY, i, type)
        ...
        with open('./fig2/%s' % type + '/CNNGRU/results.txt', 'a') as file:
