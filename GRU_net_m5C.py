...
def main(type, numm, para, xlsname):
    if os.path.exists('5results.txt'):
        print("The result will be written in the 'results.txt' file")
    else:
        with open('5results.txt', 'w') as file:
    ...
    for i in range(1, 6):
        trainX, trainY, input, _ = data_processing.load_data('./data_train/m5C/onehot/trainX%d.pickle' % i)
        valX, valY, _, _ = data_processing.load_data('./data_train/m5C/onehot/valX%d.pickle' % i)
        testX, testY, _, _ = data_processing.load_data('./data_train/m5C/onehot/testX%d.pickle' % i)
        ...
        with open('5results.txt', 'a') as file:
            file.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + '\t' + str(type) + '\t' + str(i) + '\t' +
                       str(loss1) + '\t' + str(acc1) + '\t' + str(loss2) + '\t' + str(acc2) + '\t' +
                       str(pre_score[0]) + '\t' + str(pre_score[1]) + '\t' + str(pre) + '\t' + str(rec) + '\t' +
                       str(SN) + '\t' + str(SP) + '\t' + str(f1) + '\t' + str(mcc) + '\t' + str(roc_auc) + '\n')
