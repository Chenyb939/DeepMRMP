...
def fine_tuning(m1AtrainX, m1AtrainY, m1AvalX, m1AvalY, i, type, modelname):
    ...
    checkpointer = ModelCheckpoint(filepath='./fig1/%s' % type + '/transfer/model/' + str(type) + '_%d.h5' % i, verbose=1,
                                   save_best_only=True, monitor='val_loss', mode='min')
...
def evaluate_model(modelname, testX, testY, i, type):
    ...
    # fileX = open('./fig1/%s' % type + '/transfer/pre_score%d.pickle' % i, 'wb')
    ...
    fileX = open('./fig1/%s' % type + '/transfer/pred_proba%d.pickle' % i, 'wb')
    ...
    plt.savefig('./fig1/%s' % type + '/transfer/curve/' + str(type) + 'Precision-Recall%d.png' % i)
    ...
    plt.savefig('./fig1/%s' % type + '/transfer/curve/' + str(type) + 'ROC %d.png' % i)
...
def print_fine_loss(fitHistory, i, type):
    ...
    plt.grid(True)
    plt.legend(['train', 'validation'], loc='upper left')
    plt.savefig('./fig1/%s' % type + '/transfer/curve/' + str(type) + 'loss%d.png' % i)
...
def main(type):
    ...
    for i in range(1, 2):
        ...
        pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model('./fig1/%s' % type + '/transfer/model/' + str(type) + '_%d.h5' % i,
                                                                       testX, testY, i, type)
