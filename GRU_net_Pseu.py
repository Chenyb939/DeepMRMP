...
def evaluate_model(modelname, testX, testY, i, type):
    ...
    fileX = open('./pre_score/pre_score%d.pickle' % i, 'wb')
...
def main(type, numm, para, xlsname):
    if os.path.exists('presults.txt'):
        print("The result will be written in the 'results.txt' file")
    else:
        with open('presults.txt', 'w') as file:
    ...
    for i in range(1, 6):
        trainX, trainY, input, _ = data_processing.load_data('./data_train/Pseu/onehot/trainX%d.pickle' % i)
        valX, valY, _, _ = data_processing.load_data('./data_train/Pseu/onehot/valX%d.pickle' % i)
        testX, testY, _, _ = data_processing.load_data('./data_train/Pseu/onehot/testX%d.pickle' % i)

        cnn = model(input, numm)
        history = fit_model(trainX, trainY, valX, valY, cnn, i, type)
        # history = fine_tuning(trainX, trainY, valX, valY, i, type, './model/m6A_gru_1.h5')
        ...
        with open('presults.txt', 'a') as file:
            file.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + '\t' + str(type) + '\t' + str(i) + '\t' +
                       str(loss1) + '\t' + str(acc1) + '\t' + str(loss2) + '\t' + str(acc2) + '\t' +
                       str(pre_score[0]) + '\t' + str(pre_score[1]) + '\t' + str(pre) + '\t' + str(rec) + '\t' +
                       str(SN) + '\t' + str(SP) + '\t' + str(f1) + '\t' + str(mcc) + '\t' + str(roc_auc) + '\n')
